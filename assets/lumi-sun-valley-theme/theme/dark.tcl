...
namespace eval ttk::theme::lumi_sv_dark {
  package provide ttk::theme::lumi_sv_dark 2.5
  ...
  proc load_images {imgfile} {
    variable I
    image create photo lumi_spritesheet_dark -file $imgfile -format png
    foreach {name x y width height} $::spriteinfo {
      set I($name) [image create photo -width $width -height $height]
      $I($name) copy lumi_spritesheet_dark -from $x $y [expr {$x+$width}] [expr {$y+$height}]
    }
  }

  load_images [file join [file dirname [info script]] spritesheet_dark.png]

  ttk::style theme create lumi-sun-valley-dark -parent clam -settings {
    ...
    ttk::style map Accent.TButton -foreground [list pressed "#f4edf7" disabled "#a5a5a5"]
